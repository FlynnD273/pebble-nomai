...
void floorDivMod(int32_t numerator, int32_t denominator, int32_t *floor,
                 int32_t *mod) {
...
void fctx_set_fill_color(FContext *fctx, GColor c) { fctx->fill_color = c; }
...
void edge_init(Edge *e, FPoint *top, FPoint *bottom) {
  ...
  if (e->height) {
    ...
    int32_t initialNumerator =
        dM * 16 * e->y - dM * top->y + dN * top->x - 1 + dN * 16;
...
void fctx_init_context_bw(FContext *fctx, GContext *gctx) {
  ...
  if (frameBuffer) {
    ...
    fctx->flag_buffer =
        gbitmap_create_blank(fctx->flag_bounds.size, GBitmapFormat1Bit);
...
void fctx_plot_circle_bw(FContext *fctx, const FPoint *fc, fixed_t fr) {
  ...
  if ((fc->x - fr) < fctx->extent_min.x)
    fctx->extent_min.x = fc->x - fr;
  if ((fc->y - fr) < fctx->extent_min.y)
    fctx->extent_min.y = fc->y - fr;
  if ((fc->x + fr) > fctx->extent_max.x)
    fctx->extent_max.x = fc->x + fr;
  if ((fc->y + fr) > fctx->extent_max.y)
    fctx->extent_max.y = fc->y + fr;
...
void fctx_end_fill_bw(FContext *fctx) {
  ...
  if (rowMin < 0)
    rowMin = 0;
  if (rowMax >= fctx->flag_bounds.size.h)
    rowMax = fctx->flag_bounds.size.h - 1;
  if (colMin < 0)
    colMin = 0;
  if (colMax >= fctx->flag_bounds.size.w)
    colMax = fctx->flag_bounds.size.w - 1;
  ...
  for (row = rowMin; row <= rowMax; ++row) {
    ...
    GBitmapDataRowInfo flagRowInfo =
        gbitmap_get_data_row_info(fctx->flag_buffer, row);
...
void edge_init_aa(Edge *e, FPoint *top, FPoint *bottom) {
  ...
  if (e->height) {
    ...
    int32_t dM = bottom->x - top->x;
    int32_t initialNumerator =
        dM * F * e->y - dM * top->y + dN * top->x - 1 + dN * F;
...
void fctx_plot_edge_aa(FContext *fctx, FPoint *a, FPoint *b) {
  ...
  while (edge.height > 0 && edge.y <= max_y) {
    ...
    GBitmapDataRowInfo row =
        gbitmap_get_data_row_info(fctx->flag_buffer, pixelY);
    if (pixelX < row.min_x) {
...
void fctx_plot_circle_aa(FContext *fctx, const FPoint *c, fixed_t r) {
  ...
  /* Expand the bounding box of pixels drawn. */
  if ((c->x - r) < fctx->extent_min.x)
    fctx->extent_min.x = c->x - r;
  if ((c->y - r) < fctx->extent_min.y)
    fctx->extent_min.y = c->y - r;
  if ((c->x + r) > fctx->extent_max.x)
    fctx->extent_max.x = c->x + r;
  if ((c->y + r) > fctx->extent_max.y)
    fctx->extent_max.y = c->y + r;
...
bool fctx_is_aa_enabled() { return fctx_init_context == &fctx_init_context_aa; }
...
static void bezier(FContext *fctx, fixed_t x1, fixed_t y1, fixed_t x2,
                   fixed_t y2, fixed_t x3, fixed_t y3, fixed_t x4, fixed_t y4) {
...
void fctx_curve_to_func(FContext *fctx, FPoint *params) {
  bezier(fctx, fctx->path_cur_point.x, fctx->path_cur_point.y, params[0].x,
         params[0].y, params[1].x, params[1].y, params[2].x, params[2].y);
  fctx->path_cur_point = params[2];
}

typedef void (*fctx_draw_cmd_func)(FContext *fctx, FPoint *params);

void fctx_transform_points(FContext *fctx, uint16_t pcount, FPoint *ppoints,
                           FPoint *tpoints, FPoint advance) {
  ...
  while (dst != end) {
    fixed_t x = (src->x - fctx->transform_pivot.x + advance.x) *
                fctx->transform_scale_to.x / fctx->transform_scale_from.x;
    fixed_t y = (src->y - fctx->transform_pivot.y + advance.y) *
                fctx->transform_scale_to.y / fctx->transform_scale_from.y;
    ...
    if (dst->x < fctx->extent_min.x)
      fctx->extent_min.x = dst->x;
    if (dst->y < fctx->extent_min.y)
      fctx->extent_min.y = dst->y;
    if (dst->x > fctx->extent_max.x)
      fctx->extent_max.x = dst->x;
    if (dst->y > fctx->extent_max.y)
      fctx->extent_max.y = dst->y;
...
static void exec_draw_func(FContext *fctx, FPoint advance,
                           fctx_draw_cmd_func func, FPoint *ppoints,
                           uint16_t pcount) {
  FPoint tpoints[3];
...
void fctx_draw_path_with_buffer(FContext *fctx, FPoint *points, FPoint *buffer,
                                uint32_t num_points) {
...
void fctx_draw_commands(FContext *fctx, FPoint advance, void *path_data,
                        uint16_t length) {
